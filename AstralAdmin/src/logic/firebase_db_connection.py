...
from firebase_admin import credentials, exceptions, firestore
...
async def put_new_user(author_id: str, guild_id: str, rsi_handle: str, display_name: str, user_verification_code: str):
    ...
    try:
        ...
        user_ref.collection("user_guilds").document(str(guild_id)).set({"verified": False, "verified_on": None})
        return True
...
async def put_new_guild(guild_id: str, guild_name: str, spectrum_id: str):
    ...
    try:
        ...
        guild_ref.set({"guild_name": guild_name, "guild_spectrum_id": spectrum_id, "guild_id": guild_id})
        return True
    except exceptions.FirebaseError as exc:
...
async def update_user_verification_status(author_id: str, user_verification_progress: int, user_verification_status: bool):
    ...
    try:
        ...
        user_ref.update({"user_verification_progress": user_verification_progress, "user_verification_status": user_verification_status})
        return True
    except exceptions.FirebaseError as exc:
...
async def update_user_guild_verification(author_id: str, guild_id: str, guild_verification_status: bool):
    ...
    try:
        ...
        guild_ref.set({"verified": guild_verification_status, "verified_on": firestore.SERVER_TIMESTAMP, "user": author_id})
        return True
    except exceptions.FirebaseError as exc:
