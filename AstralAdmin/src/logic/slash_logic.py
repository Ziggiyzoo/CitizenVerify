...
class SlashCommandsLogic():
    ...
    async def check_db_for_discord_user(
        self,
        user,
        author_id: int,
        rsi_handle: str,
        ...
        if user_info is None:
            ...
            if await firebase_db_connection.check_rsi_handle(rsi_handle, guild_id):
                ...
                logger.warn("The Discord User is trying to bind with an already bound RSI Handle.")
                try:
                    await bot.get_channel(1269758666416979970).send(
                      f"Discord User has tried to sign up with already bound RSI Handle {rsi_handle}."
                    )
                    return 0
                except discord.errors.Forbidden as exc:
                    logger.warn("The Bot does not have permission to message in this channel.")
                    return 0
            else:
    ...
    async def check_db_for_rsi_user(
        rsi_handle
        ):
        """
    ...
    async def validate_user(
      ...
      if await rsi_lookup.verify_rsi_handle(
          rsi_handle=user_db_info["rsi_handle"],
          verification_code=user_db_info["user_verification_code"]
          ):
          ...
          await firebase_db_connection.update_user_guild_verification(
            author_id=author_id,
            guild_id=guild_id,
          ...
          try:
              await update_user_roles.update_user_roles(
                user_list=user_list,
                bot=bot,
                guild_id=guild_id
                )
