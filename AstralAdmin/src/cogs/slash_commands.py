...
class SlashCommands(commands.Cog):
    ...
    @commands.slash_command(name="get-validation-code", description="Get the Validation Code to symbolically bind your RSI and Discord Accounts")
    async def get_verification_code(self, ctx, rsi_handle: discord.Option(str)):
        """
        ...
        if rsi_user_json is None:
            await ctx.followup.send("Please input your RSI Handle.")
        elif rsi_user_json == "Response Not OK":
            await ctx.followup.send("The API for RSI's User pages is currently down. Please try again later or contact an Admin")
        else:
            ...
            if result == 0:
                ...
                logger.error(
                    "There was an error in the binding of the following Discord and RSI Accounts. " + "Discord: %s, RSI Handle: %s.",
                    author_name,
                    rsi_handle,
                )
                ctx.followup.send(
                    "An Error was encountered adding your Discord User to the system. " + "Please try again later, or contact an Admin."
                )
            elif result == 1:
                ...
                await firebase_db_connection.update_user_verification_status(
                    author_id=author_id, user_verification_status=False, user_verification_progress=1
                )
    ...
    @commands.slash_command(name="validate", description="Continue the validation process. This command checks for your code on your RSI User Page.")
    async def validate(self, ctx):
        ...
        if user_db_info is None or validation_progress == 0:
            ...
            ctx.followup.send(f"Greetings {author_name}, you have not received a Validation Code yet. " + "Please run, /get-validation-code first.")
    ...
    @commands.slash_command(name="apply-now", description="Assistance with applying to the Astral Dynamics Organistation.")
    async def apply_now(self, ctx):
        ...
        await ctx.respond(
            f"Hi there {ctx.author.mention}. To Apply to Astral Dynamics please use this link:"
            + "\n\nhttps://robertsspaceindustries.com/orgs/ASTDYN"
            + "\n\nOnce you have done this please @ mention Human Resources.",
            ephemeral=True,
        )

    @commands.slash_command(name="add-guild", description="Add the Discord Guild to the DB.")
    ...
    async def add_guild(self, ctx, spectrum_id: discord.Option(str)):
        ...
        if await firebase_db_connection.put_new_guild(str(ctx.guild_id), ctx.guild.name, spectrum_id):
            await ctx.respond("This Discord server has been added to the Database.", ephemeral=True)
        else:
            await ctx.respond("The Discord server failed to be added to the Database", ephemeral=True)

    @commands.slash_command(name="del-guild", description="Delete the Discord Guild from the DB.")
    @commands.has_permissions(administrator=True)
    async def del_guild(self, ctx):
        ...
        if await firebase_db_connection.del_guild(guild_id=str(ctx.guild_id)):
            await ctx.respond("The Discord server has been removed from the Database", ephemeral=True)
        else:
            ctx.respond("The Discord server failed to be removed from the Database", ephemeral=True)

    @commands.slash_command(name="update-roles", description="Update the roles of bound members on the discord.")
    @commands.has_permissions(administrator=True)
